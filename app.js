...
const S = {
  ...
  async list(p) { try { const r = await window.storage.list(p); return r ? r.keys : []; } catch { return []; } }
};
...
async function init() {
  ...
  if (saved) D = Object.assign(JSON.parse(JSON.stringify(DEF)), saved);

  /* Coba init Firebase (tidak block kalau gagal) */
  await initFirebase();
...
async function save() { return S.set('tjkt1-app', D); }
...
/* ═══════════════════════════════════════════
   MUSIC — YouTube "17" Tulus (background)
   ═══════════════════════════════════════════ */
/* Video ID YouTube untuk "17" - Tulus */
...
/* YouTube IFrame API callback (dipanggil otomatis oleh YT SDK) */
function onYouTubeIframeAPIReady() {
  ytPlayer = new YT.Player('ytFrame', {
    height: '1', width: '1',
    videoId: YT_VIDEO_ID,
    playerVars: {
      autoplay: 0, loop: 1, playlist: YT_VIDEO_ID,
...
function updateMusicUI() {
  ...
  if (bar && ytPlaying) bar.classList.add('visible');
}
...
/* nextSong tidak relevan (1 lagu loop), tapi dipertahankan agar HTML tidak error */
function nextSong() { if (ytReady && ytPlayer) ytPlayer.seekTo(0); }
...
function startSinceLulus() {
  function tick() {
    const now  = new Date();
    const diff = now - LULUS_DATE;        // ms sejak lulus
