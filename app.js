...
/* ═══════════════════════════════════════════
   MUSIC — MP3 lokal (17 - Tulus)
   Taruh file MP3 di folder yang sama dengan index.html
   Nama file default: lagu.mp3
   ═══════════════════════════════════════════ */

/* Nama file MP3 — ganti kalau nama filenya beda */
const MP3_FILE = 'lagu.mp3';
...
function initMusic() {
  ...
  audio.addEventListener('play',  () => { isPlaying = true;  updateMusicUI(); });
  audio.addEventListener('pause', () => { isPlaying = false; updateMusicUI(); });
  audio.addEventListener('ended', () => { isPlaying = false; updateMusicUI(); }); /* loop=true jadi jarang terpanggil */

  audio.addEventListener('error', () => {
    console.warn('⚠ File lagu.mp3 tidak ditemukan. Taruh file MP3 di folder yang sama.');
    const bar = document.getElementById('musicBar');
    if (bar) bar.classList.remove('visible');
  });

  /* Auto-play saat user pertama interaksi dengan halaman */
  const tryPlay = () => {
...
/* nextSong tidak relevan (1 lagu), dipertahankan agar HTML tidak error */
function nextSong() {
...
/* ═══════════════════════════════════════════
   GALERI
   ═══════════════════════════════════════════ */
/* ── Simpan array item galeri aktif untuk navigasi lightbox ── */
let lbItems  = [];
let lbCurrent = 0;
...
function renderGaleri(filter) {
  ...
  /* Simpan untuk lightbox navigation */
  lbItems = all.filter(x => x.url || x.data); /* hanya yang punya media */
  ...
  all.forEach((x, i) => {
    ...
    /* Index di lbItems untuk lightbox */
    const lbIdx = lbItems.findIndex(it => it === x);

    let inner;
    if      (url && x.type === 'video') inner = `<video src="${url}" style="width:100%;height:100%;object-fit:cover" muted playsinline preload="metadata"></video>`;
    ...
    const clickAttr = lbIdx >= 0 ? `onclick="lbOpen(${lbIdx})"` : '';
    g.innerHTML += `
      <div class="${cls}" ${clickAttr}>
        ${inner}
        ${x.type === 'video' ? '<div class="vbadge">VIDEO</div><div class="play-icon">▶</div>' : ''}
...
/* ═══════════════════════════════════════════
   LIGHTBOX — foto gede & video player
   ═══════════════════════════════════════════ */
function lbOpen(idx) {
  lbCurrent = idx;
  lbShow();
...
function lbShow() {
  ...
  /* Stop video sebelumnya kalau ada */
  media.querySelectorAll('video').forEach(v => { v.pause(); v.currentTime = 0; });

  if (item.type === 'video') {
    media.innerHTML = `
      <video controls autoplay playsinline
        style="max-width:90vw;max-height:80vh;border-radius:12px;outline:none;box-shadow:0 20px 60px rgba(0,0,0,.6)">
        <source src="${url}">
        Browser tidak mendukung video.
      </video>`;
  } else {
    media.innerHTML = `
      <img src="${url}" alt="${esc(item.label)}"
        style="max-width:90vw;max-height:80vh;border-radius:12px;object-fit:contain;box-shadow:0 20px 60px rgba(0,0,0,.6)">`;
  }

  caption.textContent = item.label + (item.sub && item.sub !== item.label ? ' — ' + item.sub : '');
  counter.textContent = (lbCurrent + 1) + ' / ' + lbItems.length;

  /* Sembunyikan nav kalau hanya 1 item */
...
function lbClose(e) {
  /* Klik di luar media = tutup. Klik pada media = biarkan */
  if (e && e.target && (e.target.tagName === 'IMG' || e.target.tagName === 'VIDEO')) return;
